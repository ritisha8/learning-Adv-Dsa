...
int knapsack(vector<int> wt,vector<int> val,int W,int n){
    ...
    // remember how we did in sorting array mid of stack , we make input small by taking the last ellemt out and then storing it somewhere  doign a  recursibve call for reduced input , here we dothe same 
    ...
    //now we memorise it , memorise means we will store it in matrix in order to store the output of recrusive call, so that if the same rec call is made in any other part of recusrive tree then it will just take value form the matrix andf then extra overhead will be reduced
    ...
    if(wt[n-1]<=W){  // here we check if wt of item is smaller than cap
        return t[n][W]=max(val[n-1]+knapsack(wt,val,W-wt[n-1],n-1),knapsack(wt,val,W,n-1));  // then we took two choices one of including val[n-1], when we include n-1 in knapsack then ofc its cap for further checing will decrease because that space is occupied by the wt of this elemnt we chose ,
        // and another choice we make of not inluding the ellemtn and just dogn a recyrsive call for n-1 elelmtns , and now we put bothinto max function , because we want o return the ,ax profit , as we have two choices the ,ax of 2 will be returned
    }
    else//then we check if  weight of n-1 ellemtns is greater than cap then, simply dont include the eelemnt swe simply make a recursie call to one choicethatw e have not nt including
    {
