...
int solve(int e, int f){
    ...
    for(int k=1;k<=f;k++){
        int temp=1+max(solve(e-1,k-1),solve(e,f-k));  //we have taken maximum of when egg will break and when it will not because we want to return the worst case, so
